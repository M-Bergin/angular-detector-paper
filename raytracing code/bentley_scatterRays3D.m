...
function [singleScattered,all_dirs,numScattersRay,initPos,surfImg] = bentley_scatterRays3D(N, deltax, hrms, lambda, init_theta, n_rays, varargin)
    if (nargin < 5)
    ...
    %Flip the normals around the correct way
    fnorm(1,:)=-fnorm(1,:);
    fnorm(2,:)=-fnorm(2,:);

    surface = TriagSurface(vertices, fdef', fnorm',lattice, fmat', materials);
    z_shift=-max(surface.vertices(:,3));
%     surface.moveBy([0, 0, z_shift]);
%    figure;
%    surface.patchPlot(false);

%     centroid_x=mean(reshape(vertices(fdef(:),1),size(fdef)));
%     centroid_y=mean(reshape(vertices(fdef(:),2),size(fdef)));
%     centroid_z=mean(reshape(vertices(fdef(:),3),size(fdef)));%+ z_shift;
% 
% 
%     hold on
%     quiver3(centroid_x,centroid_y,centroid_z,fnorm(1,:),fnorm(2,:),fnorm(3,:))
    ...
    parfor i=1:poolsize
        ...
        itpos = [unifrnd(-surface_size*3/16,surface_size*3/16, 2, n); zeros(1,n)];
        initposses{i} = itpos;
        itpos = itpos - 10*itdr;
